...
def moveR(board, blank_cell_idx, num_cols):
    ...
    board[blank_cell_idx - 1], board[blank_cell_idx] = (
...
def moveD(board, blank_cell_idx, num_cols):
    if blank_cell_idx < num_cols:
        return blank_cell_idx
    board[blank_cell_idx - num_cols], board[blank_cell_idx] = (
        board[blank_cell_idx],
        board[blank_cell_idx - num_cols],
    )
...
def ShowStartInterface(screen, width, height):
    ...
    title = tfont.render("Pokemon puzzle", True, RED)
...
def main():
    ...
    pygame.display.set_caption("Puzzle Pokemon")
    ...
    while is_running:
        ...
        for i in range(num_cells):
            ...
            y_pos = i % num_cols
            rect = pygame.Rect(
                y_pos * cell_width, x_pos * cell_height, cell_width, cell_height
            )
